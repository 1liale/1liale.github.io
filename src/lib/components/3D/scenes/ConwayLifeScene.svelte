<script lang="ts">
  ...
  import { Stars, interactivity, InstancedMesh } from "@threlte/extras";
  ...
  function addPatterns(grid: Grid, patternsToAdd: any[]): Grid {
    for (const { type, count } of patternsToAdd as {
      type: PatternType;
      count: number;
    }[]) {
      ...
      row.map((cell, j) => {
        ...
        return {
          ...
          position: [
            ...
            cell.position[1],
            (newAlive
              ? gridSettings.cellSize / 2
              : -gridSettings.cellSize / 2) + cell.height,
          ],
        };
      })
    );
  ...
  function updateCellHeights() {
    ...
    const hoverChanged =
      JSON.stringify(hoverPosition) !== JSON.stringify(previousHoverPosition);
    ...
    for (let i = 0; i < gridSettings.size; i++) {
      for (let j = 0; j < gridSettings.size; j++) {
        grid[i][j].height +=
          (grid[i][j].targetHeight - grid[i][j].height) * smoothingFactor;
        grid[i][j].position[2] =
          (grid[i][j].alive
            ? gridSettings.cellSize / 2
            : -gridSettings.cellSize / 2) + grid[i][j].height;
      }
    }
  }
        ...
        for (let i = 0; i < gridSettings.size; i++) {
          for (let j = 0; j < gridSettings.size; j++) {
            ...
            const waveOffset = Math.max(
              0,
              waveAmplitude * (1 - Math.abs(distance - waveProgress))
            );
  ...
  $: opacity = Math.max(0, 1 - Math.pow(scrollY / 500, 1.5));
</script>
...
<InstancedMesh id="inner">
	<T.BoxGeometry args={[gridSettings.cellSize, gridSettings.cellSize, gridSettings.cellSize]} />
