<script lang="ts">
	...
	let cards: { opacity: number; position: THREE.Vector3; rotation: THREE.Euler, isSelected: boolean }[];
	...
	const updateCardOpacities = () => {
		...
		cards = cards.map((card, index) => {
			const cardPosition = new THREE.Vector3().copy(card.position).applyMatrix4(carouselGroup.matrixWorld);
			...
			const opacity = Math.max(0.4, 1 - Math.pow(normalizedDistance, smoothFactor));
			return { ...card, opacity };
		});
	};
	...
	useTask((delta) => {
		...
		if (isRotating) {
			...
			if (isTransitioningCamera) {
				const cameraPositionDiff = new THREE.Vector3().subVectors(targetCameraPosition, camera.position);
				...
				if (selectedCardPosition) {
					...
					angleToRotate = (angleToRotate >= 0 ? angleToRotate : (2 * Math.PI + angleToRotate));
				...
				let shortestDiff = ((diff % (2 * Math.PI)) + 3 * Math.PI) % (2 * Math.PI) - Math.PI;
...
<T.PerspectiveCamera makeDefault focus={1.5} position={[0, 0, 4.25]} fov={75} bind:ref={camera}>
	<OrbitControls
		enablePan={false}
